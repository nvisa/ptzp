...
void IRDomeDriver::configLoad(const QString filename)
{
	if (!QFile::exists(filename)) {
		...
		QJsonDocument doc;
		QJsonObject o;
		o.insert("model", QJsonValue::fromVariant("Ekinoks"));
		if (getHeadCount() > 0) {
			o.insert("type" , QJsonValue::fromVariant("moving"));
			o.insert("pan_tilt_support", 1);
			o.insert("ir_led_support", 1);
		}
		else {
			o.insert("type" , QJsonValue::fromVariant("fixed"));
			o.insert("pan_tilt_support", 0);
			o.insert("ir_led_support", 0);
		}
		o.insert("cam_module", QJsonValue::fromVariant("PV8430_F2D"));
		doc.setObject(o);
		QFile f(filename);
